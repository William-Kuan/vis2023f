...
function _inner_circle_question(artist){return(
Object.keys(artist[0])[1]
)}

function _outer_circle_question(artist){return(
Object.keys(artist[0])[16]
)}

function _data(artist,inner_circle_question,outer_circle_question,buildHierarchy)
{
  var inner_circle_answer = artist.map(row => row[inner_circle_question]);
  var outer_circle_answer = artist.map(row => row[outer_circle_question]);

  // combine inner & outer
  var combined_answers = inner_circle_answer.map((innerAns, index) => innerAns + '-' + outer_circle_answer[index]);

  var reformatted_answers = combined_answers.map(item => {
    const [prefix, values] = item.split('-');
    const split_values = values.split(';').map(value => value.trim());
    
    return split_values.map(value => `${prefix}-${value}`);
  }).reduce((acc, curr) => acc.concat(curr), []);

  var answer_counts = {};
  reformatted_answers.forEach(reformatted_ans => {
    answer_counts[reformatted_ans] = (answer_counts[reformatted_ans] || 0) + 1;
  });


  var csv_data = Object.entries(answer_counts).map(([answer, count]) => [answer, String(count)]);
  

  return buildHierarchy(csv_data);
}
...
function _sunburst(partition,data,d3,radius,inner_circle_question,outer_circle_question,width,color,arc,mousearc)
{
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("inner_circle_question")).define("inner_circle_question", ["artist"], _inner_circle_question);
  main.variable(observer("outer_circle_question")).define("outer_circle_question", ["artist"], _outer_circle_question);
  main.variable(observer("data")).define("data", ["artist","inner_circle_question","outer_circle_question","buildHierarchy"], _data);
  ...
  main.variable(observer("viewof sunburst")).define("viewof sunburst", ["partition","data","d3","radius","inner_circle_question","outer_circle_question","width","color","arc","mousearc"], _sunburst);
